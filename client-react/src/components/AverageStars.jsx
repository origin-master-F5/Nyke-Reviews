    ...
    for (let i = 1; i <= 5; i++) {
      if (fullStars) {
        if (props.averageRating >= i) {
          starArray.push(<div className="star-filled-jr" style={{ clipPath: `polygon(50% 0%, 61% 35%, 98% 35%, 68% 57%, 79% 91%, 50% 70%, 21% 91%, 32% 57%, 2% 35%, 39% 35%)`, background: "black" }} />)
        } else {
          fullStars = false
        ...
        starArray.push(<FontAwesomeIcon icon={faStar} style={{ color: "grey" }} />)
      }
